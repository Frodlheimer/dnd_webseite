...
import { renderRulesTextWithLinks } from './rulesTextLinks';
...
export const ClassDocumentBlocks = ({
  blocks,
  currentEntryId,
  currentClassId,
  spellLinkState,
  onSpellLinkClick
}: ClassDocumentBlocksProps) => {
  const renderText = (text: string) => {
    const options: {
      ...
      allowSingleWordSubclassAliases: false
    };

    if (currentEntryId) {
      options.currentEntryId = currentEntryId;
    }
    if (currentClassId) {
      options.currentClassId = currentClassId;
    }
    if (spellLinkState) {
      options.spellLinkState = spellLinkState;
    }
    if (onSpellLinkClick) {
      options.onSpellLinkClick = onSpellLinkClick;
    }

    return renderRulesTextWithLinks(text, options);
  };
  const renderTableCellText = (text: string) => {
    const options: {
      currentEntryId?: string;
      currentClassId?: string;
      allowSingleWordSubclassAliases?: boolean;
      ...
      allowSingleWordSubclassAliases: true
    };
  ...
  return (
    <div className="space-y-4 text-sm leading-relaxed text-slate-200">
      {blocks.map((block, index) => {
        ...
        if (block.type === 'p') {
          return (
            <p key={key} className="text-[0.98rem] leading-8 text-slate-200">
