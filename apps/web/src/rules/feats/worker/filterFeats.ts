import type { FeatEntryMeta, FeatsPackIndex } from '../types';

export type FeatsTagGroups = {
  collection: string[];
  source: string[];
  race: string[];
  ability: string[];
...
export const normalizeFeatsQuery = (value: string): string => {
  return value
    .toLowerCase()
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
...
export const buildFeatsTagGroups = (allTags: string[]): FeatsTagGroups => {
  const groups: FeatsTagGroups = {
    collection: [],
    source: [],
    race: [],
    ability: [],
  ...
  for (const tag of allTags) {
    ...
    if (tag.startsWith('race:')) {
      groups.race.push(tag);
      continue;
    }
    if (tag.startsWith('ability:')) {
      groups.ability.push(tag);
      continue;
    }
...
export const filterFeatsWithIndex = (args: {
  ...
  const normalizedQuery = normalizeFeatsQuery(args.query);
  const tags = new Set<string>(args.selectedTags.map((entry) => entry.trim()).filter(Boolean));

  tags.add('kind:feat');

  if (args.raceFilter && args.raceFilter !== 'ALL') {
    tags.add(`race:${args.raceFilter}`);
  }

  if (args.abilityFilter && args.abilityFilter !== 'ALL') {
    tags.add(`ability:${args.abilityFilter.toLowerCase()}`);
  }
  ...
  for (const candidateIndex of candidates) {
    ...
    if (
      ...
      !normalizeFeatsQuery(meta.summary).includes(normalizedQuery)
    ) {
      continue;
    }
